let raw = [], view = [], activeSource = 'all', activeTag = 'all';
const $ = sel => document.querySelector(sel);
const listEl = $('#list'), emptyEl = $('#empty'), searchEl = $('#search');
const sourcesEl = $('#sources'), sortEl = $('#sort'), randomBtn = $('#random'), tagsEl = $('#tags');
...
function bindEvents(){
  ...
  tagsEl.addEventListener('click', e=>{
    const t = e.target.closest('.tag'); if(!t) return;
    [...tagsEl.children].forEach(n=>n.classList.remove('active'));
    t.classList.add('active'); activeTag = t.dataset.tag; applyAndRender();
  });
...
function applyAndRender(){
  ...
  view = raw.filter(x=>{
    ...
    const matchT = activeTag==='all' || (x.tags||[]).includes(activeTag);
    return matchQ && matchS && matchT;
  });

  const [key, order] = sortEl.value.split('-');
  view.sort((a,b)=>{
...
function renderTags(list){ tagsEl.innerHTML = list.map(t=>`<span class="tag ${t==='all'?'active':''}" data-tag="${t}">${t}</span>`).join(''); }
